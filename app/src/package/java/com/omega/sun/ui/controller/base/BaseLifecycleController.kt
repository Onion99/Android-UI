...
import com.omega.sun.event.PageMessenger
import com.omega.sun.ui.ContainerActivity
import com.omega.sun.ui.controller.HomeController
...
//                        if(BuildConfig.DEBUG) ToastUtils.showShort("${controller::class.simpleName} 重新恢复可见")
    ...
    fun somethingChangeVisible(visible: Boolean){
        ...
        if(visible && (activity as? ContainerActivity)?.router?.checkControllerIsInTop(this) == true && !AppUtils.isAppForeground()){
    ...
    private fun releaseResource(){
        if(!hasReleaseResource){
            ...
            // ---- 移除监听Observers ------
            PageMessenger.getResult().removeObservers(this)
            // ---- 减少性能消�?�，取消当前协程 ------
